...
15. ☐ IS 데이터가 BS와 동일한 FY 연도 범위를 커버하는지 확인 (BS: FY2012~2024이면 IS도 동일)
16. ☐ 매출/영업이익/당기순이익 값이 모든 FY 연도에 존재하는지 (None 없이)
17. ☐ 단위 보정 확인: IS 매출(백만원) × 1,000,000 → BS 자산(원)과 동일 단위
18. ☐ IS/CIS 병합이 건너뛰어졌는지 확인 (HTML IS + CIS 단위 불일치 방지)
19. ☐ 이전 연도 매출/영업이익 추이가 합리적인지 (급격한 단절 없이 연속적)
