...
**모든 검증의 최종 단계로 프론트엔드 검증 스크립트를 반드시 실행해야 한다.**
프론트엔드 검증이 완료되어야 검증 완료로 간주한다.

```bash
# 프론트엔드 검증 (로그인 → 검색 → 추출 → 데이터 비교)
cd /home/servermanager/pers-dev2
PYTHONUNBUFFERED=1 python3 -u test_frontend_verify.py http://localhost:8000

# dev2 서버 대상
PYTHONUNBUFFERED=1 python3 -u test_frontend_verify.py http://localhost:8002
```

**검증 항목:**
- IS 원본에 있는 데이터가 VCM에도 표시되는지 (연도별 누락 감지)
- 핵심 IS 항목(매출/영업이익/당기순이익/EBITDA) 연도 커버리지
- BS 균형(자산총계 = 부채와자본총계)
- 판관비 누락 여부

**검증 기업 (6개):**
패스트파이브(01290406), CJ대한통운(00113410), 삼성전자(00126380),
E1(00165583), 노랑풍선(00632845), 한화에어로스페이스(00126566)
...
#### 브라우저 렌더링 검증 (Playwright)

**API 검증과 별도로, 실제 브라우저(Playwright headless Chromium)에서 렌더링된 결과를 검증한다.**

```bash
# 브라우저 렌더링 검증 (로그인 → 검색 → 추출 → BS/IS 탭 분리 → 툴팁 → 음수 스타일)
cd /home/servermanager/pers-dev2
PYTHONUNBUFFERED=1 python3 -u test_frontend_browser.py http://localhost:8000

# dev2 서버 대상
PYTHONUNBUFFERED=1 python3 -u test_frontend_browser.py http://localhost:8002
```

**검증 항목 (8개):**
| # | 검증 | 판정 |
|---|------|------|
| 1 | 로그인 성공 | FAIL if 모달 안 닫힘 |
| 2 | 기업 검색·선택 | FAIL if 결과 없음 |
| 3 | 추출 완료 (vcm-table 렌더링) | FAIL if 5분 타임아웃 |
| 4 | BS 테이블에 IS 전용 항목 없음 | FAIL if IS 항목 존재 |
| 5 | IS 테이블에 BS 전용 항목 없음 + 매출 행 존재 | FAIL if BS 항목 존재 or 매출 없음 |
| 6 | BS 균형 (자산총계 = 부채와자본총계, diff ≤ 2) | FAIL if 불균형 |
| 7 | 툴팁(.calc-cell) 존재 + 세부내역/계산식 키워드 | WARN if 없음 |
| 8 | 음수 셀에 빨간색(#dc2626) 스타일 | WARN if 없음 |

**합격 기준:** FAIL=0, ERROR=0 필수. WARN은 허용.
...
5. **프론트엔드 값이 최종 기준 (★최우선★)**
   - API 테스트(localhost)와 프론트엔드(pers.moatai.app)의 값이 다를 수 있음
   - **API 통과 ≠ 프론트 통과**. 반드시 실제 웹 프론트(pers.moatai.app)에서 추출하여 확인
   - Playwright 또는 직접 브라우저에서 값을 눈으로 확인해야 검증 완료
   - API로만 확인하고 "PASS"라고 하면 안 됨 — 프론트 캐시, 서버 재시작 누락, 코드 경로 차이 등으로 다른 결과 가능
...
**섹션별 합계 검증 (★필수★):**
15. ☐ 유동자산 sub-items 합 = 유동자산 총계 (±2 허용, GAP > 1% 시 FAIL)
16. ☐ 비유동자산 sub-items 합 = 비유동자산 총계 (±2 허용)
17. ☐ 유동부채 sub-items 합 = 유동부채 총계 (±2 허용)
18. ☐ **비유동부채 sub-items 합 = 비유동부채 총계** (비유동차입부채 누락 시 수조원 GAP 발생 — V2 분류 동일계정명 유동/비유동 구분 실패 사례 있음)

**EBITDA/Net Debt 합리성 검증 (★필수★):**
19. ☐ **EBITDA D&A > 0 확인**: 유형자산+사용권자산 보유 기업에서 감가상각비=0이면 BUG (Notes 미추출)
20. ☐ **D&A 연도간 연속성**: D&A가 특정 연도만 0이거나 전년 대비 50% 이상 급변 시 조사
21. ☐ **Net Debt 합리성**: 비유동부채 > 비유동부채 sub-items 합이면 비유동차입부채 누락 가능 → Net Debt 과소계상
22. ☐ **V2 동일계정명 분류 검증**: 차입금/리스부채 등이 유동/비유동 양쪽에 동일 이름으로 존재할 때 양쪽 모두 분류되었는지

**BS/IS 탭 분리 검증 (★필수★):**
23. ☐ 재무상태표 탭: 유동자산~Net Debt까지만 표시 (IS 항목 혼입 없음)
24. ☐ 손익계산서 탭: 매출~EBITDA까지 표시 (BS 항목 혼입 없음)
25. ☐ **매출=0인 기업**: 손익계산서 탭이 비어있지 않은지 확인 (매출원가부터라도 표시)
26. ☐ 좌우→상하 변환 정상: 엑셀 Financials 시트가 side-by-side면 프론트에서 분리 표시

**V1→V2 회귀 검증: BS 필수 항목 존재 (★V2 전환 후 반드시★):**
V1에서 하드코딩으로 보장했던 항목들이 V2(LLM 분류)에서도 빠짐없이 생성되는지 확인.
**수정 시 기존에 잘 되던 기업들이 깨지지 않도록 회귀 테스트를 반드시 수행.**

27. ☐ **유동자산 하위**: 현금및현금성자산, 매출채권및기타채권, 재고자산, 단기투자자산, 당기법인세자산, 기타유동자산 — 해당 원본 데이터가 있으면 반드시 존재
28. ☐ **비유동자산 하위**: 유형자산, 무형자산, 사용권자산, 장기투자자산, 투자부동산, 기타비유동자산 — 해당 원본 데이터가 있으면 반드시 존재
29. ☐ **유동부채 하위**: 유동차입부채(단기차입금+유동성장기+유동성사채+전환사채), 매입채무및기타채무, 기타금융부채, 기타유동부채, 당기법인세부채
30. ☐ **비유동부채 하위**: **비유동차입부채(장기차입금+사채)**, 기타비유동금융부채, 기타비유동부채, 이연법인세부채, 퇴직급여채무 — 특히 비유동차입부채 누락 주의 (CJ대한통운 2조원 GAP 사례)
31. ☐ **자본 하위**: 자본금, 이익잉여금, 기타자본구성요소(자본잉여금+자본조정+기타포괄손익누계액), 비지배지분
32. ☐ **파생 항목**: NWC(유동자산-유동부채), Net Debt(유동차입부채+비유동차입부채-현금-단기투자), EBITDA(영업이익+감가상각비+무형자산상각비)

**V1→V2 회귀 검증: IS 필수 항목 존재:**
33. ☐ **IS 핵심**: 매출, 매출원가, 매출총이익, 판매비와관리비, 영업이익, 법인세비용차감전이익, 당기순이익 — 원본에 있으면 반드시 존재
34. ☐ **영업외**: 영업외수익(금융수익+기타수익+지분법손익), 영업외비용(금융비용+기타비용) — 구성요소별 분리 확인
35. ☐ **% of Sales 행**: 매출총이익률, 영업이익률, 순이익률, EBITDA 마진 — 계산 정상 여부

**V1→V2 회귀 검증: EBITDA 구성요소 (★핵심★):**
36. ☐ **EBITDA 툴팁에 감가상각비 > 0**: 유형자산 or 사용권자산 보유 기업에서 감가상각비=None/0이면 BUG
37. ☐ **Notes→VCM 파이프라인**: Notes에서 D&A 추출(로그 `[VCM-v2] 주석 감가상각비:`) → Frontdata 저장(`[EBITDA]감가상각비`) → VCM display(`EBITDA`) 전 단계 값 일치 확인
38. ☐ **Notes 연도 커버리지**: `추출된 FY=` 로그에서 요청 연도 전체 포함 확인 (CJ대한통운 FY2023 누락 사례)
39. ☐ **SGA fallback**: Notes 감가상각비=None이면 SGA 상세에서 추출했는지 확인

**V1→V2 회귀 검증: Net Debt 구성요소:**
40. ☐ **비유동차입부채 > 0**: 비유동부채에 사채/장기차입금이 있는 기업에서 비유동차입부채=0이면 BUG
41. ☐ **동일계정명 유동/비유동 구분**: DB 캐시에서 `차입금`, `리스부채`, `사채` 등이 current_liability와 non_current_liability 양쪽에 분류되었는지 확인
42. ☐ **Net Debt = 유동차입부채 + 비유동차입부채 - 현금 - 단기투자**: 각 구성요소가 0이 아닌 합리적 값인지

**V2 수정 시 회귀 보호 원칙 (★최우선★):**
- **기존 419개 기업 검증 결과를 깨뜨리면 안 됨** — 새 수정은 반드시 기존 통과 기업 재검증
- 동일계정명 구분 수정 시: 유동 분류가 기존대로 유지되는지 먼저 확인 후 비유동 추가
- Notes 파이프라인 수정 시: FY2021/2022에서 잘 되던 D&A 추출이 여전히 동작하는지 확인
- LLM 프롬프트 수정 시: 캐시 무효화 범위 최소화 (해당 기업만, 전체 캐시 초기화 금지)
- **삼성전자, E1, 패스트파이브, 한화에어로스페이스 — 회귀 테스트 필수 기업**

**HTML IS 폴백 검증 (XBRL IS가 비어있는 기업 — 현대자동차, 패스트파이브 등):**
43. ☐ IS 데이터가 BS와 동일한 FY 연도 범위를 커버하는지 확인 (BS: FY2012~2024이면 IS도 동일)
44. ☐ 매출/영업이익/당기순이익 값이 모든 FY 연도에 존재하는지 (None 없이)
45. ☐ 단위 보정 확인: IS 매출(백만원) × 1,000,000 → BS 자산(원)과 동일 단위
46. ☐ IS/CIS 병합이 건너뛰어졌는지 확인 (HTML IS + CIS 단위 불일치 방지)
47. ☐ 이전 연도 매출/영업이익 추이가 합리적인지 (급격한 단절 없이 연속적)
48. ☐ HTML IS 컬럼명 공백 처리 확인: '당 기'/'전 기' → FY 변환 정상 (VCM-v2 IS=[] 아닌지 확인)
...
| 2026-03-19 | IS 항목이 BS 탭에 표시됨 (CJ대한통운, 패스트파이브 등) | 프론트 BS/IS 분리가 '매출' 항목명에만 의존 → 매출=0이면 필터링되어 분리 불가 | ① 프론트: Net Debt 이후 첫 항목을 IS 시작으로 인식 + isStartNames 확장 ② 백엔드: 매출/매출원가/영업이익 등 IS 핵심 행 항상 유지 (값=0이어도 필터 안 함) |
| 2026-03-19 | 패스트파이브 IS 전체 비어있음 (HTML IS 폴백 실패) | DART HTML 컬럼명 '당 기'/'전 기' (공백 포함)를 FY 변환하는 regex가 공백 없는 '당기'/'전기'만 매칭 | regex에 `\s*` 추가: `당\s*기\|전\s*전\s*기\|전\s*기` + '주 석' 컬럼 스킵 추가 |
