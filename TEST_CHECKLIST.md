...
| batch25 | 20 | 15 | 14 | 1 | 0 |
| batch26 | 20 | 3 | 3 | 0 | 0 |
| **누적** | **583** | **331** | **285** | **21** | **24** |
...
- 피코그램(batch25): DART 2024/2023 연결재무제표 XBRL 없음 (2022만 성공). V1 매출=676,053은 **현금흐름표 주석의 "이자수익"을 매출로 오매칭**한 것. V2=0이 정확. → V2>V1로 재분류
...
8. H1-H4 추가 버그 수정 (2026-03-16):
   - H1: overflow 합계=0 조건 제거 — 양/음 상쇄 시 항목 소실 방지
   - H2: 동적 충돌 감지 — 다른 섹션 동명 항목(보증금 등) `[섹션명]` 접미사 자동 추가
   - H3: `[비유동]` 접미사 후 `(개별)` 비교 수정 — orig_name과도 비교
   - H4: 부채와자본총계 truthiness → `is not None` (자본총계=0 대응)
...
| **양/음 상쇄 시 overflow 소실** | 중간 | overflow 합계=0이면 컨테이너 미생성 → 항목 소실 | ✅ 수정완료 (H1: 조건 제거) |
| **다른 섹션 동명 항목** | 중간 | "보증금"이 유동+비유동 양쪽에 있으면 값 덮어쓰기 | ✅ 수정완료 (H2: 동적 충돌 감지) |
| **IS 행 구조 첫 연도 고정** | 중간 | 다른 연도 고유 SGA/영업외 항목 소실 | ✅ 수정완료 (M1: 다른 연도 고유 항목 병합) |
