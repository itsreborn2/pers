...
### [FIX] D&A fallback — Notes D&A가 유형자산 1% 미만이면 SGA만 추출로 판단, fallback 강제
- **문제**: Notes에서 SGA D&A만 추출하면 값>0 → fallback 미실행 (한국전력 241M, 유형자산 200조)
- **수정**: D&A < 유형자산의 1%이면 SGA만 추출로 판단 → fallback 강제 실행
- **결과**: fallback 실행됨. 하지만 "39. 비용의 성격별 분류" 페이지의 값도 241M — **단위 문제 (백만원 vs 천원) 또는 단일 컬럼 구조 미대응**. 미해결.
- **미해결**: 한국전력 FY2024 "비용의 성격별 분류" 페이지는 (단위:백만원) 단일 컬럼. 코드가 천원→원(*1000) 변환 적용 → 과소 계산. 백만원 단위 감지 필요.

### [FIX] D&A fallback 합계 컬럼 우선 + 키워드 "계속영업 손익" 추가:
- **한국전력/SK텔레콤 D&A SGA만 추출 → 합계 컬럼 우선 선택으로 해결**:
  - 원인: 한국전력 "비용의 성격별 분류" 테이블 헤더가 `[항목명, 판관비, 매출원가, 합계]` 구조. cells[1]=SGA(231M), cells[3]=합계(13,829M). 기존 코드가 cells[1](당기 기본값) 사용 → SGA만 추출.
  - 수정: 당기 컬럼 감지 우선순위 변경: "합계" 컬럼 > "당기" 컬럼 > 기본값 1. 헤더에 "합계"가 있으면 해당 컬럼 사용 → 전체 D&A(SGA+COGS) 추출.
- **KB금융 FY2024 None**: 사업보고서 존재 확인. 금융업 XBRL IS 추출 문제 (별도 조사 필요).
