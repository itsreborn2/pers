...
### [FIX] 사용권자산 검색을 BS 원본에서 직접 수행 (LLM 분류 의존 제거)
- **문제**: EBITDA D&A fallback 조건에서 사용권자산 보유 여부 확인 시 category_items/group_totals 사용 → LLM group=None으로 키 없어 조건 미충족 → fallback 미실행 → FY2024/2025 D&A=0 (CJ대한통운 5회차 반복 실패)
- **원인**: category_items의 키가 'name'인데 'display_name'으로 검색, group_totals에 group=None 항목 키 없음 등 LLM 분류 데이터 구조에 의존하는 코드가 반복적으로 실패
- **수정**: `bs_accounts`에서 '사용권' 포함 계정을 직접 검색 → BS 원본 DataFrame에서 값 확인. LLM 분류와 무관하게 동작.
- **설계 원칙**: LLM은 계정 분류(유동자산/비유동부채 등) 담당. 단순 존재 확인은 원본 데이터에서 직접 수행이 안전.
- **교훈**: LLM 분류 결과의 데이터 구조(키 이름, group 유무)에 의존하는 조건은 반복적으로 실패. 원본 데이터에서 직접 확인하는 것이 유일하게 안정적.
